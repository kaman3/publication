...
<? if(!isset($data)) : ?>
   <div class = 'message mp'>Список пуст. Вы не просмотрели ни одного объявления</div>
<? else : ?>
   <div class = 'message mp'>Здесь Вы можете ознакомиться со всеми просмотренными позициями.</div>
<? endif; ?>
...
foreach($data as $value):
    ...
    */
    ...
    if($value['idCat'] == 9 and $value['price']){
        $rent = 'в месяц';
    }else if($value['idCat'] == 10 and $value['price']){
        $rent = 'за сутки';
    }else{
        $rent = '';
    }
    ...
    <div class = 'BoxRealtyObject' itemtype = 'http://schema.org/Offer' item = '<?=$value['id'];?>' >
